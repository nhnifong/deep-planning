"""
    Same as SdA.py but in 1Gb chunks that fit in gpu memory
"""
import cPickle
import gzip
import os
...
class SdA(object):
    ...
    def pretraining_function(self, train_set_x, layern, batch_size):
        ''' Generates a function, implementing one step in trainnig the dA
        corresponding to the layer with given index.
    ...
    for ley in xrange(sda.n_layers): # BEEP
        ...
        for epoch in xrange(pretraining_epochs):
            ...
            for fname in os.listdir(os.path.join(data_dir,'train')):
                ...
                # be sure to 'sudo stop lightdm' before running. It won't fit alongside the GUI
                # The borrow parameter prevents theano from ever copying the data except to put it on the GPU.
                # you could mutate it in one place and it would lazily reflect it on the other device eventually.
                # however these training batches are assumed to remain unchanged
                # calling del shared_x when finished will free the memory in the GPU and main memory
                
                n_train_batches = num_datapoints // batch_size
